...
resource "aws_secretsmanager_secret" "mysql_secrets_manager" {
  ...
  name        = "std15/mysql/secrets" # 콘솔에 표시될 이름

  # 기본값(30일)이면 destroy 후에도 이름이 예약돼 있어 같은 이름으로 다시 만들 수 없다.
  # ("a secret with this name is already scheduled for deletion" 에러)
  # 실습 환경이라 즉시 삭제되도록 0 으로 둔다.
...
resource "aws_serverlessapplicationrepository_cloudformation_stack" "mysql_rotation" {
  ...
  name = "${local.tag_header}rds-mysql-rotation-stack"
...
resource "aws_rds_cluster" "mysql_cluster" {
  ...
  db_cluster_instance_class = "db.m8gd.large"
