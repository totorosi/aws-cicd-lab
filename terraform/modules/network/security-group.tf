...
resource "aws_security_group" "ssh_sg" {
  ...
  ingress {
    ...
    to_port   = 22
    protocol  = "tcp"
    # 전체 공개 대신 허용 대역만 지정합니다. 값을 비우면 아무도 접속할 수 없습니다.
...
resource "aws_security_group" "eks_node_sg" {
  ...
  ingress {
    ...
    # 출발지를 지정하지 않으면 AWS가 규칙을 만들지 않아 plan 에 계속 diff 가 남습니다.
    # 컨트롤 플레인 ENI 가 VPC 서브넷에 위치하므로 VPC CIDR 을 허용합니다.
    cidr_blocks = [aws_vpc.this.cidr_block]
    # EKS 클러스터 생성 후에는 아래처럼 클러스터 보안 그룹만 허용하도록 좁히는 것을 권장합니다.
